...
def partial_func(task_args):
    ...
    """
    index, hash_dict, output_dir,headers = task_args
...
def main():
    ...
    # invalid_urls = list(executor.map(partial_func, [(key, hash_dict, args.output_dir,headers) for key in list(hash_dict.keys())]))

    invalid_urls = set()

    with open("C:/Users/Administrator/Desktop/invalid hashes.txt", "r",encoding="utf-8") as file:
        lines = file.readlines()

        for line in lines:

            invalid_urls.add(f'https://i.imgur.com/{line.strip()}.jpg')
            tot_evals+= 1
